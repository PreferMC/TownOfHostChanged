...
class ChatCommands
{
    ...
    public static bool Prefix(ChatController __instance)
    {
        ...
        foreach (var listener in ListenerManager.GetListeners())
            if (!listener.OnOwnerSendChat(__instance))
                return false;
        ...
        {
            var cmd = CommandManager.DeleteCommandSymbol(args[0]);
            ...
            switch (args[0])
            {
                ...
                case "/dis":
                    ...
                    switch (subArgs)
                    {
                        ...
                        default:
                            __instance.AddChat(PlayerControl.LocalPlayer, "crewmate(crew) | impostor(imp)");
                            cancelVal = "/dis";
                            break;
                    }
...
/*
                ...
                case "/help":
                    canceled = true;
                    Utils.SendMessage("", PlayerControl);
                    break;
*/
                case "/m":
                case "/myrole":
    ...
    public static void OnReceiveChat(PlayerControl player, string text)
    {
        ...
        foreach (var command in CommandManager.GetCommands())
            if ((command.Name.ToLower().Equals(cmd.ToLower()) ||
                CommandManager.IsArgsContainsString(cmd.ToLower(), command.Aliases)) && !command.HostOnly)
                command.OnExecute(player, CommandManager.DeleteFirstArg(args));
