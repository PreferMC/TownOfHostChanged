...
class ChangeRoleSettings
{
    public static void Postfix(AmongUsClient __instance)
    {
        ...
        {
            listener.OnGameStarted(__instance);
        }
...
class SelectRolesPatch
{
    public static void Prefix()
    {
        ...
        if (Options.CurrentGameMode != CustomGameMode.HideAndSeek)
        {
            ...
            foreach (var role in NewRole.RoleManager.GetRoles())
            {
                ...
                AssignDesyncRole(role.CustomRole, AllPlayers, senders, rolesMap, BaseRole: role.BaseRole);
            }
    ...
    public static void Postfix()
    {
        ...
        else
        {
            ...
            foreach (var role in NewRole.RoleManager.GetRoles())
                if (!Map.ContainsKey(role.CustomRole) && !role.SubRole)
                    Map.Add(role.CustomRole, role.BaseRole);
            ...
            foreach (var value in Map)
            {
                ...
                {
                    case RoleTypes.Crewmate:
                        AssignCustomRolesFromList(value.Key, Crewmates);
                        break;
                    case RoleTypes.Engineer:
                        AssignCustomRolesFromList(value.Key, Engineers);
                        break;
                    case RoleTypes.Impostor:
                        AssignCustomRolesFromList(value.Key, Impostors);
                        break;
                    case RoleTypes.Scientist:
                        AssignCustomRolesFromList(value.Key, Scientists);
                        break;
                    case RoleTypes.Shapeshifter:
                        AssignCustomRolesFromList(value.Key, Shapeshifters);
                        break;
                    case RoleTypes.GuardianAngel:
                        AssignCustomRolesFromList(value.Key, GuardianAngels);
                        break;
                }
            }
    ...
    private static void AssignLoversRoles(int RawCount = -1)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            Logger.Info("役職設定:" + player?.Data?.PlayerName + " = " + player.GetCustomRole() + " + " + loversRole, "AssignLovers");
        }
