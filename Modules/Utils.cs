...
public static class Utils
{
    public static CustomRoles? GetRoleByName(string name)
    {
        foreach (var role in NewRole.RoleManager.GetRoles())
            if (role.Name.ToLower().Equals(name.ToLower()) || role.DisplayName.ToLower().Equals(name.ToLower())) return role.CustomRole;

        foreach (CustomRoles value in Enum.GetValues(typeof(CustomRoles)))
            if (GetString(value.ToString()).ToLower().Equals(name.ToLower())
                || value.ToString().ToLower().Equals(name.ToLower()))
                return value;
